...
def check_item(item_row: dict, reddit=None, engine=None) -> dict:
    ...
    if alert is not None:
        ...
        # Item id, not name: the name is user text that the model boundary
        # may have refused or masked, and a log must not put back what the
        # guardrail took out. The summary HAS cleared the output guardrail.
        logger.info("ALERT [%s] item %s: %s", alert.severity.value, item_id, alert.summary)
