...
_BLOCKED_MESSAGE = (
    "This request was refused by Laeria's safety layer and was not sent to any "
    "model, search or payment step."
)
_OVERSIZE_MESSAGE = (
...
class BedrockGuardrails:
    ...
    def sanitize_model_output(
        ...
        replaced, plus how many were removed. Lists lose only the offending
        item; a blocked top-level string becomes empty, which is what lets the
        existing "no consensus pick means low confidence" rule do the rest of
        the work without this module knowing anything about research.
