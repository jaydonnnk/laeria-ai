...
- **The monitor worker auto-starts via a Windows Startup shortcut (`laeria-monitor.lnk`)** pointing at `run_worker.cmd`. It is easy to forget this exists and accidentally launch a second instance manually — the port-lock singleton guard (bound to localhost:47931) now prevents actual duplicate execution, but don't assume the worker isn't already running.
